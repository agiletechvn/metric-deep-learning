...
def feature_extraction_videos(model, cores, batch_sz, video_list, output_path):
    ...
    videos = {i: video.strip()
              for i, video in enumerate(open(video_list).readlines())}
    ...
    future_videos = dict()
    output_list = []
    pbar = tqdm(range(np.max(videos.keys()) + 1),
                mininterval=1.0, unit='video')
    for video in pbar:
        if os.path.exists(videos[video]):
            ...
            path = os.path.join(output_path, '{}_{}'.format(
                video_name, model.net_name))
            output_list += ['{}\t{}'.format(video_name, path)]
            pbar.set_postfix(video=video_name)

            # save features
            np.save(path, features)
    np.savetxt('{}/video_feature_list.txt'.format(output_path),
               output_list, fmt='%s')
...
def feature_extraction_images(model, cores, batch_sz, image_list, output_path):
    ...
    for batch in tqdm(range(batches), mininterval=1.0, unit='batches'):

        # load images in parallel
        future = []
        for image in images[batch * batch_sz: (batch+1) * batch_sz]:
            future += [pool.apply_async(load_image,
                                        args=[image, model.desired_size])]
...
if __name__ == '__main__':
    ...
    if args['framework'].lower() in ['cf', 'caffe']:
        ...
        model = CNN_caffe(args['network'].lower(),
