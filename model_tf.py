...
class CNN_tf():
    ...
    def __init__(self, name, model_ckpt):
        ...
        if name == 'inception':
            ...
            self.layers = ['Mixed_3a', 'Mixed_4a',
                           'Mixed_5e', 'Mixed_6h', 'Mixed_7b']
        elif name == 'resnet':
        ...
        with tf.contrib.slim.arg_scope(arg_scope):
            _, net = network(vid_processed, num_classes=None,
                             is_training=False)

        # 1. normalize on channel dimension
        # 2. global max-pooling on channel dimension
        # 3. normalize feature vector
        net = [tf.nn.l2_normalize(tf.reduce_max(tf.nn.l2_normalize(tf.nn.relu(
            net[l]), 3, epsilon=1e-15), axis=(1, 2)), 1, epsilon=1e-15) for l in self.layers]
    ...
    def vgg_preprocess(self, images):
        ...
        channels = tf.split(
            axis=ax, num_or_size_splits=num_channels, value=images)
